...
through a small set of typed, audited tools.
  ...
  python3 server.py                      # stdio transport (Claude Desktop / Claude Code / MCP Inspector)
  OPS_MCP_READ_ONLY=1 python3 server.py  # read-only: write tools are not even registered
  OPS_MCP_TRANSPORT=streamable-http python3 server.py   # HTTP transport on :8000/mcp for remote use
...
from backend import STAGES, OpsBackend

READ_ONLY = os.environ.get("OPS_MCP_READ_ONLY", "0") in ("1", "true", "yes")
AUDIT = Path(os.environ.get("OPS_MCP_AUDIT", Path(__file__).with_name("audit.jsonl")))
db = OpsBackend(Path(os.environ.get("OPS_MCP_DB", Path(__file__).with_name("data") / "ops.db")))
